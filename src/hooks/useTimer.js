...
export const useTimer = (options = {}) => {
	...
	const getElapsedStartedTime = React.useCallback(() => {
		...
		if (isStarted()) {
			return new Date().getTime() - startTime + initialTime;
		}
	...
	const getElapsedRunningTime = React.useCallback(() => {
		...
		if (isStarted()) {
			if (isPaused()) {
				return pauseTime - startTime - totalElapsedPauseTime + initialTime;
			} else {
				return new Date().getTime() - startTime - totalElapsedPauseTime + initialTime;
			}
		}
		return 0 + initialTime;
	...
	const getElapsedResumedTime = React.useCallback(() => {
		if (isRunning()) {
			return new Date().getTime() - resumeTime + initialTime;
		}
		return 0;
	...
	const getRemainingTime = React.useCallback(() => {
		...
		return 0;
	}, [periodElapsedPauseTime, nextFireTime, isPaused, isRunning, isStarted, lastFireTime, options.delay, pauseTime, startTime]);

	const start = React.useCallback(() => {
		const currentTime = new Date().getTime();
		const newNextFireTime = options.delay ? Math.max(currentTime, options.fireImmediately ? currentTime : currentTime + options.delay) : never;
